...
void my_touchpad_read(lv_indev_drv_t * indev_driver, lv_indev_data_t * data) {
  ...
  if (tp.isTouched) {
    ...
    // MATEMATYCZNE ODWRÓCENIE OSI DOTYKU
    // Odejmujemy odczyt od maksymalnej rozdzielczości ekranu
    data->point.x = 800 - tp.points[0].x;
    data->point.y = 480 - tp.points[0].y;
...
static void wifi_reset_event_cb(lv_event_t * e) {
  lv_event_code_t code = lv_event_get_code(e);
  if (code == LV_EVENT_CLICKED) {
    Serial.println("Resetowanie ustawień WiFi...");
    wm.resetSettings(); // Czyścimy pamięć
    WiFi.disconnect(true, true); // Rozłączamy i usuwamy dane z NVS
    ESP.restart(); // Restartujemy płytkę, by wrócić do ekranu konfiguracji
  }
}
...
void showBoardInfo(String name, String serial, String desc) {
  ...
  // --- PRZYCISK RESETU WIFI (W prawym górnym rogu) ---
  lv_obj_t * wifi_btn = lv_btn_create(lv_scr_act());
  lv_obj_set_size(wifi_btn, 120, 40);
  lv_obj_align(wifi_btn, LV_ALIGN_TOP_RIGHT, -20, 20);
  lv_obj_set_style_bg_color(wifi_btn, lv_color_hex(0x555555), LV_PART_MAIN);
  lv_obj_add_event_cb(wifi_btn, wifi_reset_event_cb, LV_EVENT_ALL, NULL);

  lv_obj_t * wifi_label = lv_label_create(wifi_btn);
  lv_label_set_text(wifi_label, "Zmien WiFi");
  lv_obj_center(wifi_label);

  // --- RESZTA KARTY (KOD BEZ ZMIAN) ---
  ...
  lv_obj_set_style_text_color(title, lv_color_hex(0x07E0D0), LV_PART_MAIN);
  lv_obj_set_style_pad_top(title, 20, LV_PART_MAIN);

  lv_obj_t * serialLabel = lv_label_create(card);
  lv_label_set_text(serialLabel, ("Serial: " + serial).c_str());
  lv_obj_set_style_text_color(serialLabel, lv_color_hex(0x888888), LV_PART_MAIN);
  ...
  lv_obj_set_style_text_color(descLabel, lv_color_hex(0xFFFFFF), LV_PART_MAIN);
...
buf = (lv_color_t *)heap_caps_malloc(sizeof(lv_color_t) * BUFFER_SIZE, MALLOC_CAP_SPIRAM);
  ...
  // --- Start WiFiManager ---
  WiFi.setSleep(false);
  // wm.resetSettings(); // Usuń to, aby płytka pamiętała sieć!
